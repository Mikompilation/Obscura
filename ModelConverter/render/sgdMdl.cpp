...
void DisplayPK2(const char *filename)
{
  ...
  sgdTop = (SGDFILEHEADER*) Pk2GetAddr(pakFile, 0);
  auto texturePak = (PK2_HEAD*) Pk2GetAddr(pakFile, 1);
  ...
  //ExportMdl(vectorMeshes, std::string(filename));
}
...
{
  auto pakFile = (PK2_HEAD*) ReadFullFile(filename);
  sgdTop = (SGDFILEHEADER*) GetFileInPak(pakFile, 0);
  ...
  for (auto i = 0; i < pHead->VUMeshDesc.ucNumMesh; i++)
  {
    ...
    for (auto currPointIndex = 0; currPointIndex < numPoint; currPointIndex++)
    {
      ...
      if (pHead->VUMeshDesc.MeshType.TEX == true)
      {
        ...
        mesh->uv.push_back({sgdMeshData->astData[currPointIndex].fS, 1.0f - sgdMeshData->astData[currPointIndex].fT});
      }
